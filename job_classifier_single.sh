...
#SBATCH --output=logs/acne_classifier_%j.out
#SBATCH --error=logs/acne_classifier_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=$USER@utah.edu

# Description: Train classifier model for 200 epochs with single GPU

echo "🚀 Starting job: acne_classifier"
...
EXPERIMENT_DIR="./experiments/acne_classifier_$(date +%Y%m%d_%H%M%S)"
...
if [ $? -eq 0 ]; then
    ...
    echo "📊 Check logs at: logs/acne_classifier_${SLURM_JOB_ID}.out"
else
