...
#SBATCH --output=logs/acne_diffusion_long_%j.out
#SBATCH --error=logs/acne_diffusion_long_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=$USER@utah.edu

# Description: Train diffusion model for 500 epochs (long run)

echo "🚀 Starting job: acne_diffusion_long"
...
EXPERIMENT_DIR="./experiments/acne_diffusion_long_$(date +%Y%m%d_%H%M%S)"
...
if [ $? -eq 0 ]; then
    ...
    echo "📊 Check logs at: logs/acne_diffusion_long_${SLURM_JOB_ID}.out"
else
