...
#SBATCH --output=logs/acne_diffusion_%j.out
#SBATCH --error=logs/acne_diffusion_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=$USER@utah.edu

# Description: Train diffusion model for 100 epochs with single GPU

echo "🚀 Starting job: acne_diffusion"
...
EXPERIMENT_DIR="./experiments/acne_diffusion_$(date +%Y%m%d_%H%M%S)"
...
if [ $? -eq 0 ]; then
    ...
    echo "📊 Check logs at: logs/acne_diffusion_${SLURM_JOB_ID}.out"
else
