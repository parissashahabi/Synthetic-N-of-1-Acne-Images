#!/bin/bash
#SBATCH --job-name=acne_classifier_wandb
...
#SBATCH --output=logs/acne_classifier_wandb_%j.out
#SBATCH --error=logs/acne_classifier_wandb_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=$USER@utah.edu

# Description: Train classifier model with wandb logging

echo "🚀 Starting job: acne_classifier_wandb"
...
EXPERIMENT_DIR="./experiments/acne_classifier_wandb_$(date +%Y%m%d_%H%M%S)"
...
if [ $? -eq 0 ]; then
    ...
    echo "📊 Check logs at: logs/acne_classifier_wandb_${SLURM_JOB_ID}.out"
else
