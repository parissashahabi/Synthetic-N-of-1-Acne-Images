...
#SBATCH --output=logs/acne_diffusion_multi_%j.out
#SBATCH --error=logs/acne_diffusion_multi_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=$USER@utah.edu

# Description: Multi-GPU diffusion training (requires 2+ GPUs)

echo "🚀 Starting job: acne_diffusion_multi"
...
EXPERIMENT_DIR="./experiments/acne_diffusion_multi_$(date +%Y%m%d_%H%M%S)"
...
if [ $? -eq 0 ]; then
    ...
    echo "📊 Check logs at: logs/acne_diffusion_multi_${SLURM_JOB_ID}.out"
else
