#!/bin/bash
#SBATCH --job-name=acne_classifier_resume
...
#SBATCH --output=logs/acne_classifier_resume_%j.out
#SBATCH --error=logs/acne_classifier_resume_%j.err
#SBATCH --mail-type=BEGIN,END,FAIL
#SBATCH --mail-user=$USER@utah.edu

# Description: Resume classifier training from checkpoint

echo "🚀 Starting job: acne_classifier_resume"
...
EXPERIMENT_DIR="./experiments/acne_classifier_resume_$(date +%Y%m%d_%H%M%S)"
...
if [ $? -eq 0 ]; then
    ...
    echo "📊 Check logs at: logs/acne_classifier_resume_${SLURM_JOB_ID}.out"
else
